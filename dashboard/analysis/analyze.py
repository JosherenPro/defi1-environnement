...
DATA = os.path.join(BASE, "data") if os.path.isdir(os.path.join(BASE, "data")) else os.path.join(DASHBOARD, "data")
OUT  = os.path.join(DASHBOARD, "assets", "data.json")
...
def analyze():
    ...
    pts_ll.to_file(os.path.join(DASHBOARD, "assets", "points.geojson"),
                   driver="GeoJSON")
    fc2.to_crs(4326)[["canton_nom", "region", "FRI", "fri_class", "total_pop", "n_pts", "geometry"]
                    ].to_file(os.path.join(DASHBOARD, "assets", "cantons.geojson"),
                              driver="GeoJSON")
    ...
    if not os.path.exists(links_path):
        links_path = os.path.join(DASHBOARD, "docs", "liens_data.md")
